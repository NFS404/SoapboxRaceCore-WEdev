<html>
<head></head>
<body>
	<form action="Engine.svc/PromoCode/saleGen" method="post">
		    Your token: <input type="text" name="saleManagerToken" /><br /><br /> 
			<br /> 
			Sale's name (anything you want, but related to chosen cars): <input type="text" name="saleName" /><br /><br />
			<br />Choose the cars to sale (copy their names in the forms):<br /><br />

			S_AGERA<br />
			S_AVENTADOR<br />
			S_CATERHAM_R500_SL<br />
			S_CARRERA_GT<br />
			S_CCXR<br />
			SE_MCLAREN_F1<br />
			S_RTRX<br />
			S_R8_LMS_ULTRA<br />
			S_SLR_STIRLINGMOSS<br />			
			S_VEYRON<br />
			S_ZONDA_CINQUE<br /><br />

			A_8C<br />
			A_911_GT2_05<br />
			A_911_GT2_997<br />
			A_911_GT3_RS<br />
			A_911_GT3_RS_40<br />
			A_918RSR<br />
			A_959<br />
			A_BFH_SUV<br />
			A_BOSS302_12<br />
			A_CAMARO_ZL1ELITE<br />
			A_CARRERA_S<br />
			A_CCX<br />
			A_CONTINENTAL_CONV<br />
			A_CONTINENTAL_COUPE<br />
			A_CORVETTE_CARBON<br />
			A_CORVETTE_Z06<br />
			A_CORVETTE_ZR1<br />
			A_DBS<br />
			A_DBS_VOLANTE<br />
			A_ESTOQUE<br />
			A_EVORA<br />
			A_F40<br />
			A_FORDGT<br />
			A_GALLARDO_VB<br />
			A_GT500_SS<br />
			A_GTR_VSPEC<br />
			A_HURACAN610<br />
			A_LFA<br />
			A_M1_PROCAR<br />
			A_M3_GTS<br />
			A_A_MARUSSIA_B2<br />
			A_MIURA_CONCEPT<br />
			A_MP412C<br />
			A_MURCIELAGO<br />
			A_MURCIELAGO_RDSTR<br />
			A_MURCIELAGO_SV<br />
			A_POLICEINTERCEPTOR<br />
			A_R8_52FSI<br />
			A_R34GTR_NISMO<br />
			A_REVENTON<br />
			A_SESTOELEMENTO<br />
			A_SLR_722<br />
			A_V12_VANTAGE<br />
			A_VIPER_ACR<br />
			A_XKR<br />
			A_Z4_GT3<br />
			A_ZONDA_F<br />
			A_ZONDA_F_RDSTR<br /><br />

			B_1MSERIES<br />
			B_911RSR30<br />
			B_9146GT<br />
			B_A1<br />
			B_BOSS302_69<br />
			B_BOXSTER<br />
			B_CAMARO_ZL1<br />
			B_CHALLENGER_N<br />
			B_COBRA_427_SC<br />
			B_E92<br />
			B_ELISE<br />
			B_ESCORT_MK1<br />
			B_EUROPA_S<br />
			B_GALLARDO<br />
			B_GALLARDO_SPYDER<br />
			B_GRANDCHEROKEE_SRT<br />
			B_GT40<br />
			B_GTR<br />
			B_H1ALPHA<br />
			B_HEMICUDA<br />
			B_IMPREZA_HATCH<br />
			B_LOTUSCORTINA<br />
			B_M3GTRE46<br />
			B_M6<br />
			B_M6CONV<br />
			B_MONARO<br />
			B_MUSTANGGT05<br />
			B_PANAMERA<br />
			B_R8<br />
			B_ROADRUNNER<br />
			B_SHELBYTERLINGUA<br />
			B_VIPER<br /><br />

			C_30CSL<br />
			C_370Z<br />
			C_911TURBO<br />
			C_A3<br />
			C_CAPRI_RS3100<br />
			C_CAYMAN<br />
			C_COBRADAYTONA<br />
			C_COUNTACH<br />
			C_CROWNVICTORIA<br />
			C_CTSV<br />
			C_DIABLOSV<br />
			C_ELCAMINO<br />
			C_ESCORTRSCOS<br />
			C_ETYPE<br />
			C_EXIGECUP260<br />
			C_F150_SVTRAPTOR<br />
			C_FIREBIRDFORMULA<br />
			C_FOCUS_RS<br />
			C_GT500<br />
			C_LANCEREVO9<br />
			C_LANCEREVO_X<br />
			C_LANCIADELTAHF<br />
			C_IMPREZA_SEDAN<br />
			C_M3E30<br />
			C_MB190EVO2<br />
			C_MEGANERS<br />
			C_MIURASV<br />
			C_NSX<br />
			C_QUATTRO20V<br />
			C_RX7<br />
			C_RX7_FC3S<br />
			C_RX7RZ<br />
			C_S5<br />
			C_S2000<br />
			C_SKYLINER32<br />
			C_SKYLINER34<br />
			C_STINGRAY<br />
			C_SUPRA<br />
			C_TTRS<br /><br />

			DES_135I<br />
			D_2000GTR<br />
			D_240ZG<br />
			D_300C<br />
			D_350Z<br />
			D_370Z_RDSTR<br />
			D_CAMARO<br />
			D_CHARGER_RT<br />
			D_CHARGER_SRT8<br />
			D_CHEVELLE<br />
			D_CLIO<br />
			D_COBALT<br />
			D_COROLLA_AE86<br />
			D_ECLIPSE<br />
			D_G35<br />
			D_GTI_MK1<br />
			D_IS300<br />
			D_IS350<br />
			D_ISF<br />
			D_LANCEREVO8<br />
			D_MR2<br />
			D_MX5<br />
			D_RS4<br />
			D_RX8<br />
			D_RX8_09<br />
			D_SCIONTC<br />
			D_SOLSTICE<br />
			D_Z4<br /><br />

			E_200SX<br />
			E_240SX<br />
			E_CHALLENGER_RT<br />
			E_CHARGER_RT<br />
			E_ECLIPSEGT<br />
			E_GTO_65<br />
			E_MAZDA3<br />
			E_R32<br />
			E_SCIROCCO<br />
			E_SILVIA<br /><br />
			
			Car #1: <input type="text" name="saleCar1" /><br />
			Car #2 (can be empty): <input type="text" name="saleCar2" /><br />
			Car #3 (can be empty): <input type="text" name="saleCar3" /><br />
			Car #4 (can be empty): <input type="text" name="saleCar4" /><br />
		    
			<br /><input type="submit" value="Generate the Sale!" /><br /><br />
			<i>Sale will be deactivated automatically on Tuesday</i><br /><br />
			<i>World Evolved v2 SaleGen</i>
	</form>
</body>
</html>
